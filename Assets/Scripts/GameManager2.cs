...
public class GameManager2 : MonoBehaviour
{
    ...
    private GameState2 CurrentState;
    ...
    {
        Touch touch = Input.GetTouch(0); // Sadece ilk dokunuşu alalım
        ...
        else if (touch.phase == TouchPhase.Ended)
        {
            ...
            if (Mathf.Abs(touchDelta.x) > Mathf.Abs(touchDelta.y))
            {
                if (touchDelta.x > 0)
                {
                    ...
                    CurrentState = GameState2.Moving;
    ...
    private void FixedUpdate()
    {
        if(_moveTrue)
        {
            if (CurrentState == GameState2.Moving)
            {
                ...
                if(Vector2.Distance(new Vector2(Player.position.x, Player.position.z), new Vector2(targetDest.x, targetDest.z)) < 0.1f)
                {
                    ...
                    CurrentState = GameState2.ReadyForInput;
                }
            }
        }
    }
    ...
    private IEnumerator SpawnPlatform()
    {
        ...
        while (true)
        {
            ...
            if (CurrentState != GameState2.Lost)
            {
